...
def format_table(results: List[Dict[str, Any]]) -> str:
    ...
    headers = ["Test Name", "Helpers", "Species", "Total Animals", "Density", "Ark Pos", "Score", "Time (s)"]
    col_widths = [max(len(str(h)), max((len(str(r.get(k, ""))) for r in results), default=0)) 
                  for h, k in zip(headers, ["test_name", "num_helpers", "num_species", "total_animals", 
                                            "density", "ark_pos", "score", "execution_time"])]
...
def save_csv(results: List[Dict[str, Any]], output_path: pathlib.Path):
    ...
    fieldnames = ["test_name", "num_helpers", "num_species", "total_animals", 
                  "density", "ark_pos", "score", "execution_time", "status"]
