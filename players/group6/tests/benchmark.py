...
def get_git_metadata():
    ...
    try:
        ...
        branch_name = branch_name.lower().replace("/", "_").replace("\\", "_").replace("-", "_")
    except (subprocess.CalledProcessError, FileNotFoundError, subprocess.TimeoutExpired):
        pass
...
def generate_result_filename(results_dir: pathlib.Path, commit_hash: str, branch_name: str) -> pathlib.Path:
...
def main():
    ...
    if old_results_csv.exists():
        ...
        migrated_filename = generate_result_filename(results_dir, commit_hash, branch_name)
