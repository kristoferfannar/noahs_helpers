...
def generate_maps(output_dir: pathlib.Path, clean_old: bool = True):
    ...
    if clean_old:
        old_files = [f for f in output_dir.glob("*.json") if f.name not in expected_filenames]
        if old_files:
            print(f"Removing {len(old_files)} old map file(s) that don't match current config...")
