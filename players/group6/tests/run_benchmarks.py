...
def run_benchmarks_parallel(maps_dir: pathlib.Path, num_workers: int = None) -> list:
    ...
    print(f"Running {len(map_files)} benchmarks using {num_workers} workers...", flush=True)
    start_time = time.time()
    ...
    if TQDM_AVAILABLE:
        ...
        if use_tqdm:
            ...
            with Pool(processes=num_workers) as pool:
                results = []
                for i, result in enumerate(pool.imap_unordered(run_single_benchmark, map_files), 1):
                    results.append(result)
                    elapsed = time.time() - start_time
                    print(f"  Completed {i}/{len(map_files)} [{elapsed:.1f}s elapsed]", flush=True)
    else:
    ...
    print(f"✓ All benchmarks complete in {total_time:.1f}s ({total_time/60:.1f} min)", flush=True)
